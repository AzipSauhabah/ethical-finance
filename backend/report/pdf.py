...
def _line_chart(
    ...
    if not all_v:
        d.add(String(width/2, height/2, "Pas de données", fontSize=8,
                     textAnchor="middle", fillColor=C.HexColor(GREY)))
        return d

    lo, hi = min(all_v), max(all_v)
    rng = hi - lo or 1

    def sx(i, n): return PL + W * i / max(n-1, 1)
    def sy(v): return PB + H * (v - lo) / rng
...
def _bar_chart(
    ...
    for i, (label, val) in enumerate(zip(labels, values)):
        ...
        d.add(String(PL-3, y-3, label[:22], fontSize=6, textAnchor="end",
                     fillColor=C.HexColor("#444")))
        tx = zero_x + (bw+3 if val >= 0 else -bw-3)
        d.add(String(tx, y-3, f"{val:+.1f}%", fontSize=6,
...
def _pie_chart(
    ...
    for i, (label, val) in enumerate(zip(labels, values)):
        ...
        d.add(Wedge(cx, cy, r, angle, angle-sweep,
                    fillColor=col, strokeColor=C.white, strokeWidth=1))
        mid = math.radians(angle - sweep/2)
        lx = cx + (r+14) * math.cos(mid)
        ly = cy + (r+14) * math.sin(mid)
        d.add(String(lx, ly, f"{val/total*100:.0f}%", fontSize=7,
                     textAnchor="middle", fillColor=C.HexColor("#444")))
        angle -= sweep
...
def _chart_drawdown(dd_data):
    return _area_chart([p["drawdown"]*100 for p in dd_data], RED, 500, 150,
...
def _chart_stress(stress_data):
    items = [(s.get("label", ""), s["total_return"]*100)
             for s in stress_data if s.get("total_return") is not None]
    if not items:
...
def generate_pdf(tearsheet: dict) -> bytes:
    ...
    from reportlab.platypus import (
        Flowable, HRFlowable, PageBreak, Paragraph,
    ...
    title_s   = st("T",  fontSize=24, textColor=colors.HexColor(NAVY), fontName="Helvetica-Bold", spaceAfter=8)
    subtitle_s = st("S", fontSize=12, textColor=colors.HexColor(GOLD), fontName="Helvetica", spaceAfter=4)
    section_s  = st("H", fontSize=14, textColor=colors.HexColor(NAVY), fontName="Helvetica-Bold", spaceAfter=6, spaceBefore=4)
    ...
    def p(v): return "N/A" if v is None else f"{v*100:+.2f}%"
    def f(v, d=2): return "N/A" if v is None else f"{v:.{d}f}"
    def e(v): return "N/A" if v is None else f"{v:,.2f} €".replace(",", " ")
    def hr(): return HRFlowable(width="100%", thickness=1, color=colors.HexColor(GOLD))
    ...
    # PAGE 1 — COVER
    S += [Spacer(1,4*cm), HRFlowable(width="100%",thickness=4,color=colors.HexColor(GOLD)),
          Spacer(1,.4*cm), Paragraph("ETHICAL FINANCE PLATFORM", title_s),
          Paragraph("SAUHABAH", subtitle_s), Spacer(1,.5*cm),
          ...
          Spacer(1,1*cm), HRFlowable(width="60%",thickness=.5,color=colors.HexColor(NAVY)),
          Spacer(1,5*cm), Paragraph("AVERTISSEMENT", subtitle_s),
          Paragraph(DISCLAIMER, disclm_s), Spacer(1,.5*cm),
          Paragraph(COPYRIGHT, small_s), PageBreak()]

    # PAGE 2 — EXECUTIVE SUMMARY
    S += [Paragraph("Résumé exécutif", section_s), hr(), Spacer(1,.3*cm)]
    ...
    S += [Paragraph("Performance — NAV vs Benchmark", section_s), hr(), Spacer(1,.3*cm),
          Paragraph("Évolution de la NAV comparée au benchmark.", body_s), Spacer(1,.3*cm),
          chart(_chart_nav(nav, bnch), aspect=0.40), PageBreak()]

    # PAGE 4 — DRAWDOWN
    S += [Paragraph("Analyse du drawdown", section_s), hr(), Spacer(1,.3*cm),
          Paragraph(f"<b>Max DD :</b> {p(m.get('max_drawdown'))}  "
                    f"<b>Avg DD :</b> {p(m.get('average_drawdown'))}  "
                    f"<b>Recovery :</b> {f(m.get('recovery_factor'))}", body_s),
          Spacer(1,.3*cm), chart(_chart_drawdown(dd), aspect=0.30), PageBreak()]

    # PAGE 5 — RISK
    S += [Paragraph("Métriques de risque", section_s), hr(), Spacer(1,.3*cm)]
    risk = [["Métrique","Quotidien","Annualisé (≈)"],
            ["VaR hist. 95%", p(m.get("var_95")), p((m.get("var_95") or 0)*(252**.5))],
            ["CVaR hist. 95%", p(m.get("cvar_95")), p((m.get("cvar_95") or 0)*(252**.5))],
            ["VaR hist. 99%", p(m.get("var_99")), p((m.get("var_99") or 0)*(252**.5))],
            ["CVaR hist. 99%", p(m.get("cvar_99")), p((m.get("cvar_99") or 0)*(252**.5))],
    ...
    # PAGE 6 — STRESS TESTS
    S += [Paragraph("Tests de résistance historiques", section_s), hr(), Spacer(1,.3*cm),
          chart(_chart_stress(st_), aspect=0.36), Spacer(1,.3*cm)]
    if st_:
        sd = [["Scénario","Période","Return","Max DD","Vol","Sharpe"]]
        for s in st_:
            if s.get("n_days",0) > 0:
                sd.append([s.get("label",""),
                            f"{s.get('start','')} → {s.get('end','')}",
                            p(s.get("total_return")), p(s.get("max_drawdown")),
                            p(s.get("volatility")), f(s.get("sharpe"))])
    ...
    # PAGE 7 — COSTS
    S += [Paragraph("Évolution des coûts", section_s), hr(), Spacer(1,.3*cm),
          chart(_chart_costs(cc), aspect=0.32), Spacer(1,.3*cm),
          Paragraph(f"<b>Coûts :</b> {e(cs.get('total_costs_eur'))}  "
                    f"<b>Taxes :</b> {e(cs.get('total_taxes_eur'))}  "
                    f"<b>% NAV :</b> {p(cs.get('cost_pct_nav'))}", body_s),
          PageBreak()]

    # PAGE 8 — COST BREAKDOWN
    S += [Paragraph("Décomposition des coûts", section_s), hr(), Spacer(1,.3*cm),
          chart(_chart_breakdown(bd), width_cm=10, aspect=0.65), Spacer(1,.3*cm)]
    cb = [["Poste","Montant"],
    ...
    # PAGE 9 — ALLOCATION
    S += [Paragraph("Allocation Cash vs Investi", section_s), hr(), Spacer(1,.3*cm),
          chart(_chart_allocation(ac), aspect=0.32), PageBreak()]
    ...
    if sig:
        ...
        S += [Paragraph("Significativité statistique", section_s), hr(), Spacer(1,.3*cm)]
        sd = [["Test","Stat","p-value","Sig. 5%"],
              ["Jobson-Korkie", f(jk.get("z")), f(jk.get("p"),4), "✓" if jk.get("significant") else "✗"],
              ["Alpha Jensen t-test", f(at.get("t")), f(at.get("p"),4), "✓" if at.get("significant") else "✗"],
    ...
    if pos and pos.get("positions"):
        S += [Paragraph("Positions finales", section_s), hr(), Spacer(1,.3*cm),
              Paragraph(f"<b>NAV :</b> {e(pos.get('nav_eur'))}  "
                        f"<b>Cash :</b> {e(pos.get('cash_eur'))}  "
                        f"<b>Investi :</b> {e(pos.get('invested_eur'))}  "
                        f"<b>Return :</b> {p(pos.get('total_return'))}", body_s),
              Spacer(1,.3*cm)]
        pd_ = [["Ticker","Parts","Prix €","Valeur €","Coût moy.","P&L"]]
        for tk, pp in pos["positions"].items():
            pd_.append([tk, str(pp.get("shares",0)), f(pp.get("price_eur")),
                        e(pp.get("value_eur")), e(pp.get("avg_cost")), e(pp.get("unrealised"))])
        t = Table(pd_, colWidths=[2.5*cm,2*cm,2.5*cm,3.5*cm,3*cm,3*cm])
        t.setStyle(tbl_style())
        S += [t, PageBreak()]

    # PAGE 12 — GLOSSARY
    S += [Paragraph("Glossaire financier", section_s), hr(), Spacer(1,.3*cm)]
    for entry in GLOSSARY:
        S.append(Paragraph(f"<b>{entry['term']}</b> <i>({entry['fr']})</i> — {entry['def']}", body_s))
    S += [Spacer(1,.5*cm), HRFlowable(width="100%",thickness=.5,color=colors.HexColor(NAVY)),
          Spacer(1,.3*cm), Paragraph("Avertissement réglementaire", subtitle_s),
          Paragraph(DISCLAIMER, disclm_s), Spacer(1,.3*cm), Paragraph(COPYRIGHT, small_s)]
