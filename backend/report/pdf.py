...
def _rl_line_chart(
    ...
    def sx(i, n): return pad_l + w * i / max(n - 1, 1)
    def sy(v): return pad_b + h * (v - min_v) / rng

    # Axes
    d.add(Line(pad_l, pad_b, pad_l, pad_b + h, strokeColor=rl_colors.HexColor("#cccccc"), strokeWidth=0.5))
    d.add(Line(pad_l, pad_b, pad_l + w, pad_b, strokeColor=rl_colors.HexColor("#cccccc"), strokeWidth=0.5))

    # Grille horizontale (5 lignes)
    for i in range(1, 5):
        y = pad_b + h * i / 4
        val = min_v + rng * i / 4
        d.add(Line(pad_l, y, pad_l + w, y, strokeColor=rl_colors.HexColor("#eeeeee"), strokeWidth=0.3))
        label = y_fmt(val) if y_fmt else f"{val:.0f}"
        d.add(String(pad_l - 4, y - 3, label, fontSize=6, textAnchor="end",
                     fillColor=rl_colors.HexColor(GREY)))

    # Min/Max labels Y
    d.add(String(pad_l - 4, pad_b - 3, y_fmt(min_v) if y_fmt else f"{min_v:.0f}",
                 fontSize=6, textAnchor="end", fillColor=rl_colors.HexColor(GREY)))
...
def _rl_area_chart(
    ...
    from reportlab.graphics.shapes import Drawing, PolyLine, Polygon, String, Line
    from reportlab.graphics import renderPDF
    from reportlab.lib import colors as rl_colors
    ...
    # Axes
    d.add(Line(pad_l, pad_b, pad_l, pad_b + h, strokeColor=rl_colors.HexColor("#cccccc"), strokeWidth=0.5))
    d.add(Line(pad_l, zero_y, pad_l + w, zero_y, strokeColor=rl_colors.HexColor("#999999"), strokeWidth=0.5))
...
def _rl_bar_chart(
    ...
    d.add(Line(zero_x, pad_b, zero_x, pad_b + h,
               strokeColor=rl_colors.HexColor("#999999"), strokeWidth=0.5))
    ...
    for i, (label, val) in enumerate(zip(labels, values)):
        ...
        d.add(String(pad_l - 4, y - 3, label[:20], fontSize=6, textAnchor="end",
                     fillColor=rl_colors.HexColor("#444444")))
        d.add(String(zero_x + (bar_w + 4 if val >= 0 else -bar_w - 4), y - 3,
                     f"{val:+.1f}%", fontSize=6,
...
def _rl_pie_chart(labels: list[str], values: list[float], width: float = 300, height: float = 200) -> bytes:
    """Camembert ReportLab."""
    ...
    for i, (label, val) in enumerate(zip(labels, values)):
        ...
        d.add(Wedge(cx, cy, r, angle, angle - sweep, fillColor=c, strokeColor=rl_colors.white, strokeWidth=1))
        mid = math.radians(angle - sweep / 2)
        lx = cx + (r + 15) * math.cos(mid)
        ly = cy + (r + 15) * math.sin(mid)
        pct = f"{val/total*100:.0f}%"
        d.add(String(lx, ly, pct, fontSize=7, textAnchor="middle",
                     fillColor=rl_colors.HexColor("#444444")))
        angle -= sweep
...
def _empty_chart(width: float, height: float, msg: str = "Pas de données") -> bytes:
    ...
    d = Drawing(width, height)
    d.add(String(width/2, height/2, msg, fontSize=9, textAnchor="middle",
                 fillColor=rl_colors.HexColor(GREY)))
    buf = io.BytesIO()
    renderPDF.drawToFile(d, buf, "chart")
    buf.seek(0)
...
def _chart_drawdown(dd_data: list[dict]) -> bytes:
    ...
    return _rl_area_chart(vals, RED, width=500, height=150,
...
def _chart_costs(cost_data: list[dict]) -> bytes:
    ...
    return _rl_line_chart(series, width=500, height=160,
...
def generate_pdf(tearsheet: dict) -> bytes:
    ...
    title_s   = st("Title",  fontSize=24, textColor=colors.HexColor(NAVY), fontName="Helvetica-Bold", spaceAfter=8)
    subtitle_s = st("Subtl", fontSize=12, textColor=colors.HexColor(GOLD), fontName="Helvetica", spaceAfter=4)
    section_s  = st("Sec",   fontSize=14, textColor=colors.HexColor(NAVY), fontName="Helvetica-Bold", spaceAfter=6, spaceBefore=4)
    ...
    def tbl_style(header: bool = True) -> TableStyle:
        return TableStyle([
            (("BACKGROUND", (0, 0), (-1, 0), colors.HexColor(NAVY)) if header
    ...
    risk_data = [
        ...
        ["VaR historique 95 %", fmt_pct(m.get("var_95")), fmt_pct((m.get("var_95") or 0)*(252**0.5))],
        ["CVaR historique 95 %", fmt_pct(m.get("cvar_95")), fmt_pct((m.get("cvar_95") or 0)*(252**0.5))],
        ["VaR historique 99 %", fmt_pct(m.get("var_99")), fmt_pct((m.get("var_99") or 0)*(252**0.5))],
        ["CVaR historique 99 %", fmt_pct(m.get("cvar_99")), fmt_pct((m.get("cvar_99") or 0)*(252**0.5))],
    ...
    if sig:
        ...
        sig_data = [
            ...
            ["Jobson-Korkie (Sharpe vs bench.)", fmt_f(jk.get("z")), fmt_f(jk.get("p"), 4), "✓ Oui" if jk.get("significant") else "✗ Non"],
            ["Alpha Jensen (t-test)", fmt_f(at.get("t")), fmt_f(at.get("p"), 4), "✓ Oui" if at.get("significant") else "✗ Non"],
