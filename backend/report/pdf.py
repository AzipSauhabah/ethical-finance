...
def _chart_nav_vs_benchmark(nav_data: list[dict], bench_data: list[dict]) -> bytes:
    ...
    fig.update_layout(
        ...
        legend=dict(x=0, y=1), plot_bgcolor="white",
...
def _chart_drawdown(dd_data: list[dict]) -> bytes:
    ...
    if dd_data:
        ...
        fig.add_trace(go.Scatter(
            x=x, y=y, mode="lines", name="Drawdown",
            line=dict(color=RED, width=1.5),
            fill="tozeroy", fillcolor="rgba(184,36,36,0.2)",
        ))
    fig.update_layout(
        margin=dict(l=50, r=20, t=20, b=50),
...
def _chart_costs(cost_data: list[dict]) -> bytes:
    ...
    if cost_data:
        ...
        fig.add_trace(go.Scatter(
            x=x, y=[d["costs"] for d in cost_data],
            mode="lines", name="Commissions + slippage",
            line=dict(color=NAVY, width=1.5),
            fill="tozeroy", fillcolor=f"rgba(20,35,64,0.5)",
        ))
        fig.add_trace(go.Scatter(
            x=x, y=[d["total"] for d in cost_data],
            mode="lines", name="Total (+ taxes)",
            line=dict(color=GOLD, width=1.5),
        ))
    fig.update_layout(
        margin=dict(l=50, r=20, t=20, b=50),
        xaxis_title="Date", yaxis_title="Cumul (€)",
        plot_bgcolor="white",
        xaxis=dict(showgrid=True, gridcolor="#e0e0e0"),
        yaxis=dict(showgrid=True, gridcolor="#e0e0e0"),
    )
...
def _chart_allocation(alloc_data: list[dict]) -> bytes:
    ...
    if alloc_data:
        ...
        fig.add_trace(go.Scatter(
            x=x, y=[d["invested"] for d in alloc_data],
            mode="lines", name="Investi",
            line=dict(color=NAVY, width=0),
            fill="tozeroy", fillcolor=f"rgba(20,35,64,0.7)",
            stackgroup="one",
        ))
        fig.add_trace(go.Scatter(
            x=x, y=[d["cash"] for d in alloc_data],
            mode="lines", name="Cash",
            line=dict(color=GOLD, width=0),
            fill="tonexty", fillcolor=f"rgba(184,150,47,0.7)",
            stackgroup="one",
        ))
    fig.update_layout(
        margin=dict(l=50, r=20, t=20, b=50),
...
def _chart_stress_bars(stress_data: list[dict]) -> bytes:
    import plotly.graph_objects as go
    items = [(s.get("label", s.get("scenario", "")), s["total_return"] * 100)
