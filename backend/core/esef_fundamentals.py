...
async def get_latest_filing(lei: str, client: httpx.AsyncClient) -> Optional[dict]:
    ...
    try:
        ...
        if data:
            # Trie : période la plus récente, puis FR en priorité sur GB
            def _sort_key(f):
                a = f["attributes"]
                period = a.get("period_end", "")
                country = a.get("fxo_id", "")
                fr_prio = 0 if "/FR/" in (a.get("json_url") or "") else 1
                return (-period, fr_prio)
