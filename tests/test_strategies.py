...
class TestStrategyContract:
    @pytest.mark.parametrize("strat_name", [
        s["name"] for s in strategy_registry.list_all()
    ])
    ...
    def test_weights_in_valid_range(self, strat_name, deterministic_prices):
        ...
        for ticker, w in weights.items():
            assert 0.0 <= w <= 1.0, (
                f"{strat_name}: weight for {ticker} is {w}, must be in [0, 1]"
            )
...
class TestRegistryDiscovery:
    def test_all_builtin_strategies_discovered(self):
        ...
        expected = [
            "buy_hold", "equal_weight", "momentum", "mean_reversion",
            "sma_crossover", "risk_parity", "min_variance",
            "dual_momentum", "adaptive_trend", "ml_ensemble", "epr5",
        ]
