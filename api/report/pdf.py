...
def _chart_cost_breakdown(breakdown: dict) -> bytes:
    ...
    if sum(values) > 0:
        ...
        ax.text(0.5, 0.5, "Aucun coût", ha="center", va="center",
...
def _chart_stress_bars(stress_data: list[dict]) -> bytes:
    ...
    fig, ax = plt.subplots(figsize=(7.5, 3.0), dpi=110)
    labels = [s.get("label", s.get("scenario", "")) for s in stress_data if s.get("total_return") is not None]
...
def generate_pdf(tearsheet: dict) -> bytes:
    ...
    title_s    = st("Title",   fontSize=24, textColor=colors.HexColor(NAVY),
                    fontName="Helvetica-Bold", spaceAfter=8)
    subtitle_s = st("Subtl",   fontSize=12, textColor=colors.HexColor(GOLD),
                    fontName="Helvetica",     spaceAfter=4)
    section_s  = st("Sec",     fontSize=14, textColor=colors.HexColor(NAVY),
    ...
    badge_s    = st("Badge",   fontSize=8,  textColor=colors.white,
                    backColor=colors.HexColor(NAVY), alignment=1)

    def tbl_style(header: bool = True) -> TableStyle:
        return TableStyle([
            ("BACKGROUND", (0, 0), (-1, 0), colors.HexColor(NAVY)) if header else
    ...
    if sig:
        ...
        sig_data = [
            ...
            ["Jobson-Korkie (Sharpe vs bench.)",
             fmt_f(jk.get("z")), fmt_f(jk.get("p"), 4),
             "✓ Oui" if jk.get("significant") else "✗ Non"],
            ["Alpha Jensen (t-test)",
             fmt_f(at.get("t")), fmt_f(at.get("p"), 4),
             "✓ Oui" if at.get("significant") else "✗ Non"],
            ["Sharpe — IC bootstrap 95 %",
             f"[{fmt_f(sb[0])} ; {fmt_f(sb[1])}]", "—", "—"],
        ]
