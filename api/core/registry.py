...
from api.config import (
    ETHICAL_EXCLUSION_SECTORS, ETHICAL_EXCLUSION_TICKERS,
    INTEREST_DEBT_RATIO_THRESHOLD, INTEREST_INCOME_RATIO_THRESHOLD,
...
SHARIA_SECTOR_BLACKLIST = [
    # Riba (interest-based)
    "banks", "bank", "diversified financial", "consumer financial",
    "insurance", "capital markets", "asset management",
    # Haram products
    "alcohol", "brewers", "distillers", "wineries", "beverages alcoholic",
    "tobacco", "casino", "gambling", "resort", "hotel",
...
def run_ethical_screen(info: dict) -> EthicalScreen:
    ...
    checks.append(ScreenCheck(
        ...
        description=("Aucun secteur exclu détecté" if not matched_sectors
    ...
    checks.append(ScreenCheck(
        ...
        value=debt_ratio, threshold=INTEREST_DEBT_RATIO_THRESHOLD,
    ...
    checks.append(ScreenCheck(
        ...
        value=interest_ratio, threshold=INTEREST_INCOME_RATIO_THRESHOLD,
...
def run_sharia_screen(info: dict) -> ShariaScreen:
    ...
    checks.append(ScreenCheck(
        ...
        len(matched) == 0,
        description=("Aucune activité non-conforme détectée" if not matched
    ...
    checks.append(ScreenCheck(
        ...
        value=debt_ratio, threshold=SHARIA_DEBT_RATIO_MAX,
    ...
    checks.append(ScreenCheck(
        ...
        value=liq_ratio, threshold=SHARIA_LIQUIDITY_RATIO_MAX,
    ...
    checks.append(ScreenCheck(
        ...
        value=npi_ratio, threshold=SHARIA_NON_PERMISSIBLE_INCOME_MAX,
...
class TickerRegistry:
    ...
    async def load(self, ticker: str) -> TickerInfo:
        ...
        log.info("Loaded %s — ethical=%s sharia=%s",
                 ticker, record.is_ethical, record.is_sharia)
        return record
